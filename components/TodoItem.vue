    ...
    <div class="flex flex-row gap-2">
      <div class="flex gap-2 ml-1 flex-1 min-w-0"> <!-- Changed to flex-1 to take available space but allow shrinking -->
        <div class="flex gap-2 min-w-0 flex-1"> <!-- Added flex-1 to allow text to take remaining space -->
          ...
          <div class="text-slate-800 pt-2 dark:text-slate-300 overflow-hidden min-w-0" :class="{ 'line-through': todo.completed, 'opacity-50': todo.completed, 'truncate': !expandedText }">
            {{ todo.text }}
          </div>
        </div>
        <div class="text-slate-500 pt-2 dark:text-slate-300 inline-block whitespace-nowrap flex-shrink-0" :class="{ 'line-through': todo.completed, 'opacity-50': todo.completed }">
          {{ formattedDate }}
        </div>
      </div>
      <div class="flex flex-col items-end gap-2 text-sm justify-between pt-1.5 pr-0.5 flex-shrink-0">
        ...
        <button class="text-red-700 bg-red-300 dark:hover:bg-red-500 rounded-full shadow-sm mb-0.5 p-1.5 hover:bg-red-500 hover:text-white" v-if="expandedText" @click="emit('delete-todo', todo.id)">
          delete
        </button>
      </div>
    </div>
